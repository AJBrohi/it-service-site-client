...
const ManageOrders = () => {
    ...
    useEffect(() => {
        fetch('https://damp-shelf-97362.herokuapp.com/orders')
            .then(res => res.json())
            .then(data => setOrderList(data))
    }, []);
    // console.log(serviceList);
    const handleUpdate = (id, newStatus) => {
        fetch(`https://damp-shelf-97362.herokuapp.com/getOrder/${id}`)
        .then (res => res.json())
        .then(data => {
            setOrders(data);
        })

        fetch(`https://damp-shelf-97362.herokuapp.com/updateOrder/${id}?status=${newStatus}`, {
            method: 'PATCH',
